...
class Process(object):
    ...
    def read_string(self, lp_base_address: int, max_length: int = 256) -> str:
        """
        Read a string from the process's memory.

        :param lp_base_address: The process's pointer to the start of the string.
        :param max_length: The maximum length of the string to read.
        
        :return: The string from the process's memory.
        """
        try:
            ...
            ctypes.windll.kernel32.ReadProcessMemory(self.handle, lp_base_address, buffer, n_size, lp_number_of_bytes_read)

            # Return the string, assuming it's null-terminated
            return buffer.value.decode('cp1258')
        except (BufferError, ValueError, TypeError) as error:
            if self.handle:
                self.close()
            self.error_code = self.get_last_error()
...
class ReadWriteMemory:
    ...
    def get_processes_by_name(self, process_name: [str, bytes]) -> list:
        """
        :description: Get all processes by the process executable's name and return a list of Process objects.

        :param process_name: The name of the executable file for the specified processes, e.g., my_program.exe.

        :return: A list of Process objects containing information from the requested processes.
        """
        if not process_name.endswith('.exe'):
            process_name += '.exe'
        ...
        for process_id in process_ids:
            ...
            if self.process.handle:
                ...
                if ctypes.windll.psapi.GetProcessImageFileNameA(self.process.handle, image_file_name, MAX_PATH) > 0:
                    filename = os.path.basename(image_file_name.value)
                    if filename.decode('utf-8') == process_name:
                        process = self.process.__class__()  # Assuming 'self.process' is an instance of the Process class
        ...
        if not matching_processes:
            raise ReadWriteMemoryError(f'No processes found with the name "{process_name}"')
    ...
    def get_processes_by_name(self, process_name: [str, bytes]) -> list:
        ...
        """
        if not process_name.endswith('.exe'):
            process_name = process_name + '.exe'  # Ensure the name ends with '.exe'
            ...
            if self.process.handle:
                ...
                if ctypes.windll.psapi.GetProcessImageFileNameA(self.process.handle, image_file_name, MAX_PATH) > 0:
                    ...
                    if filename.decode('utf-8') == process_name:
                        self.process.pid = process_id
                        self.process.name = process_name
                        processes.append(self.process)  # Add the process to the list
                self.process.close()

        if not processes:
            raise ReadWriteMemoryError(f'No processes with name "{process_name}" found!')
